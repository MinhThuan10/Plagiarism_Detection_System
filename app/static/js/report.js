...
async function loadPDF(pdfId, type) {
    try {
        const response = await fetch(`/api/load_file@file_id=${pdfId}-type=${type}`);
        if (!response.ok) {
...
function loadFileInfo(sourceType) {
    ...
    .then((data) => {
        if (data.success) {
            ...
            document.getElementById("page_count").innerText = "Page count: " + page_count;

            const word_count = data.list_files[0].word_count; 
            document.getElementById("word_count").innerText = "Word count: " + word_count;

            const similarity = data.list_files[0].plagiarism;
            document.getElementById("similarity").innerText = similarity + "% Standard Similarity";
            document.getElementById("similarity2").innerText = similarity + "% Standard Similarity";
            ...
            data[sourceType].forEach((item, index) => {
                ...
                sourceTitleDiv.innerHTML = `
                    <span class="number" style="background-color: ${details.color};">${index + 1}</span>
                    <span class="tag" style="background-color: ${details.color};">${details.type_source}</span>
                `;
                ...
                percentageDiv.textContent = `${Math.floor((details.word_count / word_count) * 100)}%`;

                const containerText = document.createElement('div');
                containerText.className = 'container-text-school_id-'+school_id;
                containerText.style.display = 'none';

                if (typeof details.sentences === 'object' && details.sentences !== null) {
                    for (const key in details.sentences) {
                        if (details.sentences.hasOwnProperty(key)) {
                            ...
                            icon.addEventListener("click", function() {
                                fetch(`/api/remove_text_school@file_id=${fileId}-school_id=${school_id}-sentence=${key}`, {
                                    method: "POST",
                                    headers: {
                                        "Content-Type": "application/json",
                                    },
                    ...
                    console.log("details.sentences is not an object or array:", details.sentences);
                }
            ...
            const containerSourceExclude = document.getElementById("sourceContainer");
            Object.entries(data.school_exclusion_source).forEach(([index, source]) => {
                ...
                icon.addEventListener("click", function() {
                    ...
                    const sectionToRemove = document.getElementById(sectionSourceDiv.id);
                    if (sectionToRemove) {
                        sectionToRemove.remove();
                        console.log(`Đã loại bỏ trường học: ${index}`);
                    }
                    fetch(`/api/add_source_school@file_id=${fileId}-school_id=${index}`, {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                        },
            ...
            Object.entries(data.school_exclusion_text).forEach(([index, sources]) => {
                ...
                Object.entries(sources).forEach(([subIndex, source]) => {
                    ...
                    // Gán sự kiện click cho icon
                    icon.addEventListener("click", function() {
                        const sectionToRemove = document.getElementById(sectionSource.id);
                        if (sectionToRemove) {
                            sectionToRemove.remove();
                            console.log(`Đã loại bỏ trường học: ${index}-${subIndex}`); // Sử dụng khóa chính và phụ
                        }
                        fetch(`/api/add_text_school@file_id=${fileId}-sentence=${index}-${subIndex}`, {
                            method: "POST",
                            headers: {
                                "Content-Type": "application/json",
                            },
            ...
            document.getElementById("flexCheckDefault_reference").checked = reference
...
function toggleFillterApply() {
    ...
    const studentData = document.getElementById('flexCheckDefault_student').checked;
    ...
    const references = document.getElementById('flexCheckDefault_reference').checked;
    const curlybracket = document.getElementById('flexCheckDefault_quoted').checked;
