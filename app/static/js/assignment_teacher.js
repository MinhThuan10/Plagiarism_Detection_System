        ...
        if (data.success) {
            ...
            data.assignments.forEach(assignmentInfo => {
                ...
                row.innerHTML = `
                    ...
                    <td>${(assignmentInfo.student_ids).length}/${(data.classs.student_ids).length}</td>
                ...
                deleteicon.addEventListener('click', function() {
                    ...
                    deletebutton.addEventListener('click', function(){
                        fetch(`/api/delete_assignment@school=${school_id}-class=${class_id}-assignment=${assignment_id}`, {
                            method: 'DELETE',
                            headers: {
                                'Content-Type': 'application/json'
                            }
                ...
                editicon.addEventListener('click', function() {
                    ...
                    document.getElementById('assignmentName_replace').value = assignmentInfo.assignment_name
                    
                    
                    
                    document.getElementById('startDay_replace').value = formatDate(assignmentInfo.start_day);
                    document.getElementById('dueDay_replace').value = formatDate(assignmentInfo.end_day);

                    const updatebutton = document.getElementById('update_btn');
                    updatebutton.addEventListener('click', function(){
                        const assignment_name_replace = document.getElementById('assignmentName_replace');
                        const start_day_replace = document.getElementById('startDay_replace');
                        const end_day_replace = document.getElementById('dueDay_replace');

                        const convertedStartDay = convertDateFormat(start_day_replace.value  );
                        const convertedEndDay = convertDateFormat(end_day_replace.value  );

                        fetch(`/api/update_assignment@school=${school_id}-class=${class_id}}-assignment=${assignment_id}`, {
                            method: 'PUT',
                            headers: {
                                'Content-Type': 'application/json'
                            },
...
document.getElementById("button_create_asignment").addEventListener("click", function(event) {
    ...
    if (allFilled) {
        fetch(`/api/create_assignment@school=${school_id}-class=${class_id}`, {
            ...
            body: JSON.stringify({ assignmentName: assignmentName.value,
